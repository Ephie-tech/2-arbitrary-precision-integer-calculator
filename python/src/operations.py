...
def divide(a: ArbitraryInt, b: ArbitraryInt) -> Tuple[ArbitraryInt, ArbitraryInt]:
    if b.value == '0':
...
def modulo(a: ArbitraryInt, b: ArbitraryInt) -> ArbitraryInt:
...
def logarithm(base: ArbitraryInt, value: ArbitraryInt) -> ArbitraryInt:
    if base.value in ['0', '1']:
    ...
    while _is_greater_or_equal(value.value, current.value):
        current = multiply(current, base)
        result = add(result, ArbitraryInt('1'))
    
    return subtract(result, ArbitraryInt('1'))
...
def gcd(a: ArbitraryInt, b: ArbitraryInt) -> ArbitraryInt:
    """Calculate the Greatest Common Divisor of two ArbitraryInt numbers using Euclidean algorithm"""
    while b.value != '0':
        a, b = b, modulo(a, b)
    return a
